...
from utils.getPatch import get_patch,get_patch_valid
from utils.common import Z_Score_normalization

def main(root_dir_train, patchh_size):
    ...
    image_paths = [os.path.join(image_dir, f) for f in os.listdir(image_dir) if f.lower().endswith('.nii.gz')]

    # 获取掩码文件夹下所有文件的完整路径，筛选出具有指定扩展名的文件
    mask_paths = [os.path.join(mask_dir, f) for f in os.listdir(mask_dir) if f.lower().endswith('.nii.gz')]
    ...
    for idx, (image_path, mask_path) in enumerate(zip(image_paths, mask_paths)):
        ...
        image_patches, mask_patches, _ = get_patch(image, mask, patchh_size)
        ...
        for patch_idx, (image_patch, mask_patch) in enumerate(zip(image_patches, mask_patches)):
            ...
            # 设置保存路径
            image_patch_path = os.path.join(image_patch_dir, f'image_patch_{idx}_{patch_idx}')
            mask_patch_path = os.path.join(mask_patch_dir, f'mask_patch_{idx}_{patch_idx}')
...
if __name__ == "__main__":
    root_dir_train = '/home/zlx/PycharmProjects/pytorch_segment3D/data/1-50'  # 替换为您的训练数据路径
    patch_size = 64
    main(root_dir_train, patch_size)
