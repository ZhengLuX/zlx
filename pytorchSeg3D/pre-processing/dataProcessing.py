...
import logging
from utils.dataProcess import resize_volume,adjust_window,adjust_spacing
# 配置日志记录
...
image_function_map = {
    ...
    'adjust_image_window': adjust_window
}
...
def process_image_and_mask(image_dir, mask_dir, output_image_dir, output_mask_dir, image_processing_steps, mask_processing_steps, **kwargs):
    ...
    for image_file in os.listdir(image_dir):
        if image_file.endswith(('.mhd', '.nii.gz')):
            image_path = os.path.join(image_dir, image_file)
            # mask_file = image_file  # 假设图像文件和掩码文件有相同的文件名
            mask_file = image_file.replace('.img', '.label')
            ...
            try:
                ...
                output_image_path = os.path.join(output_image_dir, image_file.replace('.img', ''))
...
def main(root_dir, output_dir):
    ...
    image_processing_steps = [
        # {'name': 'resize_image', 'params': {'new_shape': (512, 512, 256)}},
        {'name': 'adjust_spacing', 'params': {'new_spacing': (1.0, 1.0, 1.0)}},
        {'name': 'adjust_image_window', 'params': {'window_level': 170, 'window_width': 600}}
    ]

    mask_processing_steps = [
        # {'name': 'resize_mask', 'params': {'new_shape': (512, 512, 256)}},
        {'name': 'adjust_spacing', 'params': {'new_spacing': (1.0, 1.0, 1.0)}}
    ]
...
if __name__ == "__main__":
    root_dir = 'E:/ImageCAS/1-50'
    output_dir = 'E:/ImageCAS/1-50/process'
